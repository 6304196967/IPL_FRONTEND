...
const OngoingAuctions = () => {
  ...
  const fetchUpcomingAuctions = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://ipl-server-mj6l.onrender.com/api/auction/upcoming', {
        method: 'GET',
        headers: {
  ...
  const handleSubmitJoinRequest = async () => {
    ...
    try {
      ...
      const response = await fetch('https://ipl-server-mj6l.onrender.com/api/auction/join', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
