  ...
  useEffect(() => {
    const fetchAuctionAndPurses = async () => {
      ...
      try {
        ...
        const auctionResponse = await fetch(`https://ipl-server-mj6l.onrender.com/api/auctionlive/${auctionId}`);
        ...
        for (let team of teams) {
          try {
            ...
            const response = await fetch(
              `https://ipl-server-mj6l.onrender.com/api/auctionlive/franchise/${teamName}?auctionId=${auctionId}`
            );
